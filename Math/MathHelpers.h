...
inline float Clamp(const float value, const float min = 0.0f, const float max = 1.0f)
{
	if (value < min)
	{
		return min;
...
inline float Loop(float value, const float min = 0.0f, const float max = 1.0f)
{
	if (min >= max)
	{
		return value;
...
inline float Lerp(float valueMin, float valueMax, float t)
{
	if (t > 1.0f)
	{
		return valueMax;
